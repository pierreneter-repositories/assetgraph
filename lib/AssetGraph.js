...
class AssetGraph extends EventEmitter {
    ...
    constructor(options) {
        ...
        this.teepee = new Teepee({
            ...
            timeout: options.teepeeTimeout || 30000
        });
    }
    ...
    addAsset(assetConfig, incomingRelation) {
        if (Array.isArray(assetConfig) || (typeof assetConfig === 'string' && !/^[a-zA-Z-\+]+:/.test(assetConfig) && assetConfig.includes('*'))) {
            throw new Error('AssetGraph#addAsset does not accept an array or glob patterns, try the addAssets method or the loadAssets transform');
        }
    ...
    /**
     * assetGraph.addAssets(assetConfigs)
     * ==================================
     *
     * Add assets to the graph.
     *
     * @param {Asset[]|String|String[]|Object|Object[]} The asset specs to add
     * @return {Asset[]} The assets instances that were added
     */
    addAssets(...args) {
        const assets = [];
        for (const assetConfig of _.flattenDeep(args)) {
            if (typeof assetConfig === 'string' && !/^[a-zA-Z-\+]+:/.test(assetConfig) && assetConfig.includes('*')) {
                assets.push(
                    ...glob.sync(pathModule.resolve(this.root ? urlTools.fileUrlToFsPath(this.root) : process.cwd(), assetConfig))
